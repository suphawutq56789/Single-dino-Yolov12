"""
Triple input dataset class for YOLOv12 Triple Input training (GREYSCALE VERSION).

This module handles loading and combining three separate images (primary, detail1, detail2)
into a single 3-channel input tensor for triple input training.
Each image is loaded as GREYSCALE (1 channel), so total is 3 channels instead of 9.
"""
...
class TripleInputDatasetGreyscale(YOLODataset):
    """
    Dataset class for triple input training - GREYSCALE VERSION.

    Extends YOLODataset to load three images (primary, detail1, detail2) as greyscale
    and combine them into a single 3-channel tensor (1 channel per image).
    """
    ...
    def __init__(self, img_path, imgsz=640, cache=False, augment=True, hyp=None, prefix="", rect=False, batch_size=16, stride=32, pad=0.0, single_cls=False, classes=None, fraction=1.0, data=None, task="detect", **kwargs):
        """Initialize triple input greyscale dataset."""
        ...
        if self.is_triple_input:
            LOGGER.info(f"Triple input GREYSCALE structure detected at {img_path}")
    ...
    def load_image(self, i, rect_mode=True):
        """Load and combine triple input images as GREYSCALE."""
        ...
        # Load the three images as GREYSCALE
        images = []
        original_sizes = []

        for img_path in [primary_path, detail1_path, detail2_path]:
            if img_path.exists():
                # Load image as GREYSCALE (cv2.IMREAD_GRAYSCALE returns single channel)
                im = cv2.imread(str(img_path), cv2.IMREAD_GRAYSCALE)
                if im is None:
                    LOGGER.warning(f"Failed to load image: {img_path}")
                    # Create a dummy greyscale image
                    im = np.zeros((640, 640), dtype=np.uint8)

                # Expand to 3D array (H, W) -> (H, W, 1)
                if len(im.shape) == 2:
                ...
                # Create a dummy greyscale image (H, W, 1)
                im = np.zeros((640, 640, 1), dtype=np.uint8)
                images.append(im)
        ...
        if len(images) == 3:
            ...
            for img in images:
                if img.shape[:2] != (target_h, target_w):
                    # For single channel, squeeze before resize and expand after
                    if img.shape[2] == 1:
                        img_2d = img.squeeze(axis=2)
                        img_resized = cv2.resize(img_2d, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
                        img = np.expand_dims(img_resized, axis=2)
                    else:
            ...
            # Combine into 3-channel image (each channel is one greyscale image)
            # Shape: (H, W, 3) where each channel represents one input
            combined = np.concatenate(resized_images, axis=2)  # Shape: (H, W, 3)
            ...
            # Return a dummy 3-channel greyscale image
            dummy = np.zeros((self.imgsz, self.imgsz, 3), dtype=np.uint8)
            return dummy, (self.imgsz, self.imgsz), dummy.shape[:2]

    def load_triple_images(self, i, rect_mode=True):
        """Override base class load_triple_images to load GREYSCALE (3 channels) instead of RGB (9 channels)."""
        # Simply call our greyscale load_image method
        return self.load_image(i, rect_mode)

    def cache_images_to_disk(self, i):
        """Cache triple input greyscale images to disk (override to handle 3-channel greyscale images)."""
        if not self.is_triple_input:
...
def build_triple_dataset_greyscale(cfg, img_path, batch, data, mode="train", rect=False, stride=32, single_cls=False, augment=False, cache=False, prefix="", classes=None, fraction=1.0):
    """Build triple input greyscale dataset."""
    ...
    if is_triple:
        LOGGER.info(f"Building TripleInputDatasetGreyscale for {mode}")
